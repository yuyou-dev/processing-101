...
class VisualBox(object):
    ...
    def createPork(self):
        r1 = self.startPoint.z + 300
        r2 = self.endPoint.z + 300
        #r1 = map(self.startPoint.z,-200,200,400,600)
        #r2 = map(self.endPoint.z,-200,200,400,600)
        y1 = self.startPoint.x
        y2 = self.endPoint.x
        x1 = self.startPoint.y
        x2 = self.endPoint.y
        #r1 = map(r1,-200,200,0,300)
        #r2 = map(r2,-200,200,0,300)
        aStart = map(x1,-200,200,-TAU / 2,TAU / 2)
        aEnd = map(x2,-200,200,- TAU /2,TAU / 2)
        bStart = map(y1,-200,200,-TAU / 8,TAU / 8)
        bEnd = map(y2,-200,200,-TAU /8,TAU / 8)
        

        self.pork = Pork(r1,r2,0,0,aStart,aEnd,bStart,bEnd)
    def updateAndDisplayPork(self):
        self.createPork()
        self.pork.display()
    def display(self,c):
        push()
